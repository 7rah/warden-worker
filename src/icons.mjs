...
export function handleIconRequest(method, pathname) {
  if (method !== "GET" && method !== "HEAD") return notFoundResponse();

  const domain = parseIconDomain(pathname);
  if (!domain) return notFoundResponse();

  return redirectToIconService(domain);
}

function parseIconDomain(pathname) {
...
function redirectToIconService(domain) {
  return new Response(null, {
    status: 302,
    headers: {
      Location: buildGoogleFaviconUrl(domain),
